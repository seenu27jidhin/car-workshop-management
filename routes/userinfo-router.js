...
import UserInfodetails from '../db/models/userinfo-schema.js';
...
router.get('/', checkToken(['USER']), async (req, res) => {
  try {
    // const userId = userId;
    // console.log(userId);
    const userInfo = await UserInfodetails.find();
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { gender, phoneno, dob } = req.body;
      // console.log(body);

      const userinfo = new UserInfodetails({
        gender,
        phoneno,
        dob,
        user: userId,
      ...
      const UserInfoCreate = await UserInfodetails.create(userinfo);
...
router.get('/by-user/:userId', checkToken(['USER']), async (req, res) => {
  try {
    // const userId = userId;
    // console.log(userId);
    const userInfo = await UserInfodetails.find({ user: req.params.userId });
